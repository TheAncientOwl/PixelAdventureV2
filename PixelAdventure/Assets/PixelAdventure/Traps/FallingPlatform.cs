...
namespace PixelAdventure.Traps
{
    ...
    {
        private static float k_UP_DISTANCE = 0.25f;
        private static float k_SPEED = 0.3f;
        ...
        {
            if (other.gameObject.CompareTag(Player.TAG) && PlayerMovement.Instance.IsGrounded && !m_Falling)
                Invoke("Fall", 0.5f);
        }
